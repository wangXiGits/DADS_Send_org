...
import com.cloud.send.observer.util.HttpClientUtil;
 ...
 * sendFlag : 0->1
 */
public class SportsDataSendThread implements Runnable {
	private static String className = "SportsDataSendThread";
	private static String tableName = "sports";
	...
	public void run() {
		...
		while(runFlag)
		{
			...
			{
				map = iterator.next();
//				if(Constants.DATATYPE_STEPDETAIL.equals(map.get("dataType"))||Constants.DATATYPE_STEPEFFECTIVE.equals(map.get("dataType"))||Constants.DATATYPE_STEPCOUNT.equals(map.get("dataType")))
//				{
					data = new JSONObject();
